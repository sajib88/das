...
//        $value['location'] = (!empty($postData['location']))?$postData['location']:'';
    ...
    public function teamPlayer($mid='',$tid=''){
        $this->db->select('tp.team_id, tp.player_id, tp.is_captain, tp.is_wicketkeeper, tp.is_debut, p.id, p.player_full_name,p.player_en_name');
    ...
    public function teamPlayer_11_id_ByMatch($mid='',$tid=''){
        $ids = array();
        $this->db->select('tp.team_id, tp.player_id, tp.is_captain, tp.is_wicketkeeper, tp.is_debut, p.id, p.player_full_name');
        $this->db->from('p1_crick_team_player as tp');
